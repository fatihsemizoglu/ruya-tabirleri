...
import { MessageSquare, CheckCircle, Clock, Check, Trash2, Reply, User, Calendar, ExternalLink, X } from 'lucide-react';
...
export function BlogCommentManagement() {
  ...
  const { data: commentStats } = useQuery({
    queryKey: ['admin-blog-comment-stats-aggregate'],
    queryFn: async () => {
      const [allRes, pendingRes, approvedRes] = await Promise.all([
        ...
        supabase.from('blog_comments').select('id', { count: 'exact', head: true }).eq('is_approved', true)
      ]);
      return {
        all: allRes.count || 0,
  ...
  const filteredComments = comments?.filter(comment => {
    return comment.content.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      (comment.blog_posts?.title && comment.blog_posts.title.toLowerCase().includes(searchQuery.toLowerCase()));
  }) || [];
  ...
  const bulkApproveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-blog-comments'] });
      toast.success(`${selection.selectedIds.length} yorum onaylandı`);
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-blog-comments'] });
      toast.success(`${selection.selectedIds.length} yorum silindi`);
      selection.clearSelection();
    },
    onError: (error: Error) => {
      toast.error(`Hata: ${error.message}`);
  ...
  const handleBulkApprove = () => {
    if (selection.selectedIds.length > 0) {
      bulkApproveMutation.mutate(selection.selectedIds);
    }
  };

  const handleBulkDelete = () => {
    if (selection.selectedIds.length > 0 && confirm(`${selection.selectedIds.length} yorumu silmek istediğinize emin misiniz?`)) {
  ...
  const statsData: [{ label: string; value: number; subtext: string; icon: typeof MessageSquare }, { label: string; value: number; subtext: string; icon: typeof CheckCircle }, { label: string; value: number; subtext: string; icon: typeof Clock }] = [
    { label: 'Toplam Yorum', value: commentStats?.all || 0, subtext: 'Tüm blog yorumları', icon: MessageSquare },
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        description="Blog yazısı yorumlarını inceleyin, onaylayın veya silin"
        icon={MessageSquare}
      ...
      <div className="admin-panel-surface p-6 space-y-6">
        ...
        {selection.selectedIds.length > 0 && (
          <div className="flex items-center gap-3 p-3 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <span className="text-sm font-medium">
              {selection.selectedIds.length} yorum seçildi
            </span>
            <div className="flex gap-2 ml-auto">
              <Button
                ...
                className="text-emerald-700 border-emerald-300 hover:bg-emerald-50 hover:text-emerald-800 dark:text-emerald-400 dark:border-emerald-800/60 dark:hover:bg-emerald-950/40 dark:hover:text-emerald-300"
              >
              ...
              <Button
                ...
                className="text-red-700 border-red-300 hover:bg-red-50 hover:text-red-800 dark:text-red-400 dark:border-red-800/60 dark:hover:bg-red-950/40 dark:hover:text-red-300"
              >
        ...
        {isLoading ? (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        {comment.parent_id && (
                          <Badge variant="outline" className="border-indigo-500 text-indigo-600 text-[10px] py-0 px-2 rounded-md flex items-center gap-1">
                      ...
                      {comment.blog_posts && (
                        <Link 
                          to={`/blog/${comment.blog_posts.slug}`}
                          target="_blank"
                  ...
                  <div className="flex items-center gap-3 self-end sm:self-auto flex-shrink-0">
                    {!comment.is_approved ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-xs font-semibold text-amber-600 hover:text-amber-705 dark:text-amber-400 dark:hover:text-amber-300 transition-colors"
                      >
                        <X className="w-4 h-4" />
                        Kaldır
                      </button>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs font-semibold text-red-600 hover:text-red-755 dark:text-red-400 dark:hover:text-red-300 transition-colors"
                    >
          ...
          <div className="bg-card border border-border/40 rounded-2xl">
            <EmptyState
              icon="mail"
              title="Blog yorumu bulunamadı"
              description={
                activeTab === 'pending' ? 'Onay bekleyen blog yorumu yok. Harika gidiyorsun! 🎉'
                : activeTab === 'approved' ? 'Henüz onaylanmış blog yorumu bulunmuyor.'
                : 'Aradığınız kriterlere uygun blog yorumu bulunamadı.'
              }
