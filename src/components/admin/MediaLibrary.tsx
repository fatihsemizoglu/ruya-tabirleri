...
export function MediaLibrary() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (fileName: string) => {
      const response = await mediaApi.deleteFile(fileName, 'blog-images');
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (fileNames: string[]) => {
      const response = await mediaApi.deleteFiles(fileNames, 'blog-images');
      if (!response.success) throw new Error(response.error || 'Failed to delete files');
      return fileNames;
    },
    onSuccess: (fileNames) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.admin.media });
