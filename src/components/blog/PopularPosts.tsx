...
export function PopularPosts() {
  ...
  const fetchPopularPosts = async () => {
    ...
    const response = await blogApi.getPosts({ limit: 5 });

    if (response.success && response.data) {
      let sortedData = response.data.map((post) => ({
        id: post.id,
        title: post.title,
        slug: post.slug,
      ...
      if (sortBy === 'views') {
        sortedData = sortedData.sort((a, b) => b.view_count - a.view_count);
      } else if (sortBy === 'likes') {
        sortedData = sortedData.sort((a, b) => b.like_count - a.like_count);
      } else {
