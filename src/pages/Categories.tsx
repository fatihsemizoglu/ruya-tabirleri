...
export default function Categories() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      // Fetch all dreams to calculate stats
      const dreamsResponse = await dreamsApi.getAll({ limit: 1000 });
      const dreamsData = dreamsResponse.success && dreamsResponse.data ? dreamsResponse.data : [];

      // Calculate stats per category
      const statsMap = new Map<string, { count: number; views: number }>();
      dreamsData.forEach((dream: Dream) => {
      ...
      const categoriesWithStats: CategoryWithStats[] = categoriesData.map((cat: Category) => ({
        ...cat,
        ...
        {isLoading ? (
          <div className={viewMode === 'grid' 
            ...
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="animate-pulse rounded-2xl bg-muted p-6 h-44" />
            ))}
          </div>
