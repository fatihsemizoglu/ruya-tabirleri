...
export default function History() {
  ...
  const fetchCategories = async () => {
    ...
    if (response.success && response.data) {
      setCategories(response.data.map(c => ({ id: c.id, name: c.name, slug: c.slug })));
    }
  };

  const fetchHistory = async () => {
    try {
      const response = await usersApi.getHistory(1, 100);
      ...
      const uniqueHistory = (response.data || []).reduce((acc: HistoryItem[], curr: HistoryItem) => {
        const exists = acc.find(h => h.dream_id === curr.dream_id);
        if (!exists && curr.dreams) {
          acc.push(curr);
        }
        return acc;
      }, []);
      ...
      const categoryIds = [...new Set(uniqueHistory.map(h => h.dreams?.category_id).filter(Boolean))];
      if (categoryIds.length > 0) {
        const catResponse = await categoriesApi.getAll();

        if (catResponse.success && catResponse.data) {
          const catMap = new Map(catResponse.data.map(c => [c.id, c]));
          uniqueHistory.forEach(h => {
            if (h.dreams?.category_id) {
              const cat = catMap.get(h.dreams.category_id);
              h.category = cat ? { name: cat.name, slug: cat.slug } : null;
            }
          });
        }
      }
  ...
  const clearHistory = async () => {
    try {
      const response = await usersApi.clearHistory();
  ...
  const removeItem = async (id: string) => {
    try {
      const response = await usersApi.removeHistoryItem(id);
  ...
  const filteredHistory = useMemo(() => {
    ...
    if (categoryFilter !== 'all') {
      result = result.filter(h => h.dreams?.category_id === categoryFilter);
    }
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(h => 
        h.dreams?.title.toLowerCase().includes(query) ||
        h.dreams?.content.toLowerCase().includes(query)
      );
    }
  ...
  const stats = useMemo(() => {
    ...
    const categoryCount = new Set(history.map(h => h.dreams?.category_id).filter(Boolean)).size;
                      ...
                      >
                        <Link to={`/ruya/${item.dreams?.slug}`} className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground line-clamp-1 mb-2">
                            {item.dreams?.content.substring(0, 150)}...
                          </p>
                          <div className="flex items-center gap-4 text-xs text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {(item.dreams?.view_count || 0).toLocaleString('tr-TR')}
                            </span>
                            <span className="flex items-center gap-1">
                              <Heart className="h-3 w-3" />
                              {(item.dreams?.like_count || 0).toLocaleString('tr-TR')}
                            </span>
                          </div>
                        </Link>
